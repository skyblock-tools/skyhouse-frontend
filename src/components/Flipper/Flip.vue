    ...
    <div class="flipCard card bg-neutral shadow-lg">
      ...
      <div @click="copyToClipboard" class="card-body cursor-pointer">
        <h2 class="card-title">
          ...
          <div
            class="badge badge-outline"
            v-if="rarity == 'UNCOMMON'"
            style="color: rgb(50, 255, 80)"
          >
            UNCOMMON
            <img
              v-if="recombobulated"
        ...
        <p>
          ...
          ><span class="font-mono text-2sm">{{ price }}</span>
        </p>
        <p>
          <span class="opacity-75">Est. Resell Price: </span
          ><span class="font-mono text-2sm">{{ resell_price }}</span>
        </p>
        <p>
          <span class="opacity-75">Profit: </span
          ><span class="font-mono text-2sm">{{ profit }}</span>
        </p>
        <p>
          <span class="opacity-75">House Quantity: </span
          ><span class="font-mono text-2sm">{{ house_quantity }}</span>
        </p>
        <p v-if="pet_candies > -1">
          <span class="opacity-75">Pet Candies: </span
          ><span class="font-mono text-2sm">{{ pet_candies }}/10</span>
        </p>
        <p>
          <span class="opacity-75">Time Remaining: </span>
          <vue-countdown
            :time="this.ends - Date.now()"
            :transform="transformSlotProps"
            v-slot="{ days, hours, minutes, seconds }"
          >
            <span class="font-mono"
              ><span v-if="hours > 0"
                >{{ parseInt(hours) + parseInt(days) * 24 }}h</span
