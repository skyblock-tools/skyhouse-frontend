			...
			<div tw="z-0 flex items-center justify-center max-w-7xl gap-4 p-4 col-start-1 row-start-1 ">
				<div css={[{ backgroundImage: `url(${SkyhouseShader})` }, tw`mt-10 bg-cover xl:mx-80 xl:fixed rounded-xl shadow-xl`]}>
					<div
						css={
							OwnsSkyhousePlus
								? tw`backdrop-grayscale	w-full h-full bg-zinc-900/80 py-8 px-8 rounded-xl backdrop-blur-sm`
								: tw`w-full h-full bg-zinc-900/70 py-8 px-8 rounded-xl backdrop-blur-sm`
						}>
						...
						<p tw="pt-3 font-semibold italic mb-3 text-sky-100">
							{OwnsSkyhousePlus
								? 'Thanks for supporting us by purchasing a Skyhouse+ subscription. The benefits you recive are as follows:'
						...
						{OwnsSkyhousePlus ? (
							<div tw="mt-6 text-sm inline-flex items-center text-center px-4 py-3 rounded-lg bg-sky-400 text-zinc-800 font-semibold">
								YOU ALREADY OWN SKYHOUSE+
							</div>
						) : (
							<button
								css={[
